...
class ConfigManager {
  ...
  T get<T>(String key, {T? defaultValue}) {
    ...
    if (value == null) {
      if (defaultValue != null) {
        developer.log('Config key not found, using default: $key = $defaultValue', name: 'ConfigManager');
        return defaultValue;
      }
  ...
  void set<T>(String key, T value) {
    _config[key] = value;
    developer.log('Configuration updated: $key = $value', name: 'ConfigManager');
  }
  ...
  void updateAll(Map<String, dynamic> updates) {
    ...
    developer.log('Configuration bulk update: ${updates.keys.join(", ")}', name: 'ConfigManager');
  }

  /// Enable/disable debug mode
  void setDebugMode(bool enabled) {
    set('debug_mode', enabled);
    developer.log('Debug mode ${enabled ? "enabled" : "disabled"}', name: 'ConfigManager');
  }
  ...
  void setFeatureFlag(String feature, bool enabled) {
    ...
    set(key, enabled);
    developer.log('Feature flag $feature ${enabled ? "enabled" : "disabled"}', name: 'ConfigManager');
  }
  ...
  Map<String, dynamic> getPerformanceConfig() {
    return {
      'enable_performance_monitoring': get<bool>('enable_performance_monitoring'),
      'max_cache_size': get<int>('max_cache_size'),
      'cache_ttl_minutes': get<int>('cache_ttl_minutes'),
      'network_check_interval_seconds': get<int>('network_check_interval_seconds'),
    };
  }
    ...
    try {
      ...
      for (final entry in requiredConfigs.entries) {
        ...
        if (value.runtimeType != expectedType) {
          developer.log('Invalid type for config $key: expected $expectedType, got ${value.runtimeType}', name: 'ConfigManager');
          return false;
        }
      }
      ...
      developer.log('Configuration validation failed: $e', name: 'ConfigManager');
      return false;
    }
  }
}